...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 23 05:16:18 2018
...
class Game(Widget):
    ...
    def serve_car(self): # starting the car when we launch the application
        ...
        self.car.velocity = Vector(9, 0) # the car will start to go horizontally to the right with a speed of 6
    ...
    def update(self, dt): # the big update function that updates everything that needs to be updated at each discrete time t when reaching a new state (getting new signals from the sensors)
        ...
        if sand[int(self.car.x),int(self.car.y)] > 0: # if the car is on the sand
            self.car.velocity = Vector(0.1, 0).rotate(self.car.angle) # it is slowed down (speed = 1)
            last_reward = -2 # and reward = -1
        else: # otherwise
            self.car.velocity = Vector(9, 0).rotate(self.car.angle) # it goes to a normal speed (speed = 6)
            last_reward = -0.2 # and it gets bad reward (-0.2)
            if distance < last_distance: # however if it getting close to the goal
                last_reward = 2 # it still gets slightly positive reward 0.1
